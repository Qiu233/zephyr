...
t1 :: ContextOPM B.ByteString
t1 = do
    ip <- use $ transport . device . ip_address
    uin_ <- fromIntegral <$> use uin
    Prim.t1_ uin_ ip

t2 :: String -> B.ByteString -> ContextOPM B.ByteString
t2 rst d = do
    pure $ Prim.t2_ rst d

t8 :: ContextOPM B.ByteString
t8 = do
    pure $ Prim.t8_ 2052
...
t1B :: Word32 -> Word32 -> Word32 -> ContextOPM B.ByteString
...
t33 :: ContextOPM B.ByteString
t33 = do
    guid_ <- guidBytes <$> use (transport . device . guid)
    pure $ Prim.t33_ guid_

t35 :: ContextOPM B.ByteString
t35 = do
    pure $ Prim.t35_ 8
...
t107 :: ContextOPM B.ByteString
t107 = do
    pure $ Prim.t107_ 0

t108 :: ContextOPM B.ByteString
t108 = do
...
t17C :: String -> ContextOPM B.ByteString
t17C code = do
    pure $ Prim.t17C_ code

t187 :: ContextOPM B.ByteString
t187 = do
    mac_address_ <- uses (transport . device . mac_address) utf8ToBytes
    pure $ Prim.t187_ mac_address_

t188 :: ContextOPM B.ByteString
t188 = do
    android_id_ <- uses (transport . device . android_id) utf8ToBytes
    pure $ Prim.t188_ android_id_

t191 :: Word8 -> ContextOPM B.ByteString
t191 k = do
    pure $ Prim.t191_ k

t193 :: String -> ContextOPM B.ByteString
t193 ticket = do
    pure $ Prim.t193_ ticket

t194 :: ContextOPM B.ByteString
t194 = do
    imsi_ <- use $ transport . device . imsi
    pure $ Prim.t194_ imsi_

t197 :: ContextOPM B.ByteString
t197 = do
    pure Prim.t197_

t198 :: ContextOPM B.ByteString
t198 = do
    pure Prim.t198_

t202 :: ContextOPM B.ByteString
t202 = do
...
t401 :: ContextOPM B.ByteString
t401 = do
    g_ <- use $ transport . signature . g
    pure $ Prim.t401_ g_

t511 :: ContextOPM B.ByteString
t511 = do
    let ds = [
...
t516 :: ContextOPM B.ByteString
t516 = do
    pure Prim.t516_

t521 :: Word32 -> ContextOPM B.ByteString
t521 i = do
    pure $ Prim.t521_ i

t525 :: ContextOPM B.ByteString
t525 = do
    let t536_ = Prim.t536_ $ B.pack [1, 0]
    pure $ Prim.t525_ t536_

t544 :: String -> Word32 -> ContextOPM (Either String B.ByteString)
...
t544v2 :: String -> Word32 -> ContextOPM (Either String B.ByteString)
